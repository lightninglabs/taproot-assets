...
func marshalPeerAcceptedBuyQuotes(
	...
	for scid, quote := range quotes {
		coefficient, err := quote.AssetRate.Coefficient.ToUint64Safe()
		if err != nil {
			return nil, fmt.Errorf("unable to convert coefficient "+
				"to uint64: %w", err)
		}
...
func marshalPeerAcceptedSellQuotes(quotes map[rfq.SerialisedScid]rfqmsg.SellAccept) (
	...
	for scid, quote := range quotes {
		coefficient, err := quote.AssetRate.Coefficient.ToUint64Safe()
		if err != nil {
			return nil, fmt.Errorf("unable to convert accepted "+
				"sell quote coefficient to uint64: %w", err)
		}
...
func (r *rpcServer) SendPayment(req *tchrpc.SendPaymentRequest,
	...
	case pReq.PaymentRequest != "":
		...
		rpcAssetRate := acceptedQuote.BidAssetRate

		// Return an error is the scale component of the fixed point is
		// greater than the max value of uint8.
		if rpcAssetRate.Scale > 255 {
			return fmt.Errorf("scale value overflow: %v",
				rpcAssetRate.Scale)
		}

		// Construct a BigInt fixed-point asset to BTC rate.
		assetRate := rfqmsg.NewBigIntFixedPoint(
			rpcAssetRate.Coefficient, uint8(rpcAssetRate.Scale),
		)
...
func (r *rpcServer) AddInvoice(ctx context.Context,
	...
	if acceptedQuote.AskAssetRate.Scale > math.MaxUint8 {
		return nil, fmt.Errorf("ask asset rate scale too large: %v",
			acceptedQuote.AskAssetRate.Scale)
	}

	askAssetRate := rfqmsg.NewBigIntFixedPoint(
		acceptedQuote.AskAssetRate.Coefficient,
		uint8(acceptedQuote.AskAssetRate.Scale),
	)
