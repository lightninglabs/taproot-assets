...
func (r *rpcServer) computeChannelAssetBalance(ctx context.Context,
	...
	for chanIdx := range activeChannels {
		...
		// Since the assets of the channel passed the above filter,
		// we're safe to aggregate their info to be represented as a
		// single entity.
		var aggrAssetInfo rfqmsg.JsonAssetChanInfo

		for _, assetInfo := range assetData.Assets {
			aggrAssetInfo.Capacity += assetInfo.Capacity
			aggrAssetInfo.LocalBalance += assetInfo.LocalBalance
			aggrAssetInfo.RemoteBalance += assetInfo.RemoteBalance
		}
