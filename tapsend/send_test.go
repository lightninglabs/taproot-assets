...
func createProofParams(t *testing.T, genesisTxIn wire.TxIn, state spendData,
	...
	// exclusion proofs.
	senderPrevIdKey := asset.DeriveBurnKey(
		*senderAsset.PrevWitnesses[0].PrevID,
	)
	senderScriptKey := asset.NewScriptKey(senderPrevIdKey)
	senderSTXOAsset, err := asset.NewAltLeaf(
		senderScriptKey, asset.ScriptV0,
	)
	require.NoError(t, err)
