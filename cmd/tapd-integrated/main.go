...
func (r *integratedRegistrar) RegisterGrpcSubserver(s *grpc.Server) error {
	...
	return r.tapServer.RegisterWithGrpcServer(s)
}
...
func (l *onDemandListener) Accept() (net.Conn, error) {
	if l.lis == nil {
		var err error
		l.lis, err = net.Listen(l.addr.Network(), l.addr.String())
		if err != nil {
			return nil, err
		}
	}
...
func run() error {
	...
	// configured RPC address plus a bufconn for internal communication.
	bufListener := bufconn.Listen(100)
	bufReadyChan := make(chan struct{})
	macChan := make(chan []byte, 1)

	// Build listeners for all configured RPC addresses. The first
	// listener's Ready channel is used to signal that lnd is ready.
	rpcListeners := make(
	...
	// Add the in-process bufconn listener for internal lndclient
	// communication.
	rpcListeners = append(rpcListeners, &lnd.ListenerWithSignal{
		Listener: bufListener,
		Ready:    bufReadyChan,
		MacChan:  macChan,
	})
	...
	// Step 7: Connect to lnd via bufconn for internal communication.
	lndConn, err := dialBufConn(bufListener)
	if err != nil {
		return fmt.Errorf("error connecting to lnd: %w", err)
	}
	defer lndConn.Close()

	// Step 8: Create full lnd client services via the external RPC
	...
	// Step 9: Configure the tapd server shell with lnd's services. This
	...
	// Step 10: Initialize tapd's RPC services. The gRPC services were
...
// dialBufConn creates a gRPC connection to lnd via an in-memory bufconn
// listener, skipping TLS verification since the connection is in-process.
func dialBufConn(listener *bufconn.Listener) (*grpc.ClientConn, error) {
	...
	opts := []grpc.DialOption{
		...
		grpc.WithTransportCredentials(tlsCfg),
	}

	return grpc.NewClient("", opts...) // nolint:staticcheck
}
