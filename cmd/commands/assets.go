...
func burnAssets(ctx *cli.Context) error {
	...
	if !ctx.Bool(burnOverrideConfirmationName) {
		...
		case assetSpecifier.GetAssetId() != nil:
			idBytes = assetSpecifier.GetAssetId()
			idType = "Asset ID"
			identifierHex := hex.EncodeToString(idBytes)
			...
			assetBalance, ok := balance.AssetBalances[identifierHex]
			if !ok {
		...
		case assetSpecifier.GetGroupKey() != nil:
			idBytes = assetSpecifier.GetGroupKey()
			idType = "Group Key"
			...
			groupBalance, ok := balance.AssetGroupBalances[idHex]
			if !ok {
				return fmt.Errorf("couldn't fetch balance for "+
					"group key %x", idBytes)
			}
		...
		msg := fmt.Sprintf("Please confirm destructive action.\n"+
			...
			idType, idBytes, currentBalance, burnAmount,
		)
