...
func main() {
	...
	// ---------------------------------------------------------------------
	rows, err := db.Query(`SELECT type, name, sql FROM sqlite_master WHERE type IN ('table','view') ORDER BY name`) //nolint:lll
	if err != nil {
	...
	for rows.Next() {
		...
		// Append the retrieved CREATE statement. We add a semicolon and a
		// couple of line breaks to clearly separate each object's definition.
		finalSchema += sqlDef + ";\n\n"
	}
	...
	outFile := filepath.Join(outDir, "final_schema.sql")
	if err = os.WriteFile(outFile, []byte(finalSchema), 0644); err != nil {
