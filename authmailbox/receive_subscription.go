...
// make it easier to multiplex multiple subscriptions on the same channel.
...
// newReceiveSubscription creates a new receive subscription for the mailbox
// client. All incoming messages are sent to the provided message channel, which
// MUST be buffered! The subscription is not yet active and must be
// connected/authenticated first.
func newReceiveSubscription(cfg *ClientConfig, msgChan chan<- *ReceivedMessages,
...
func (s *receiveSubscription) readIncomingStream(ctx context.Context) {
	for {
		...
		case *respTypeMessages:
			bundle := &ReceivedMessages{
				Receiver: s.receiverKey,
