...
type ChainPorterConfig struct {
	...
	ProofWatcher tapgarden.ProofWatcher
...
func (p *ChainPorter) storeProofs(sendPkg *sendPackage) error {
	...
	p.cfg.ProofWatcher.WatchProofs(
		passiveAssetRawProofs, p.updateSentProofs,
	)
...
// updateSentProofs is called by the re-org watcher when it detects a re-org
// and has updated the delivered proofs.
func (p *ChainPorter) updateSentProofs(proofs []*proof.Proof) error {
	// Let's not be interrupted by a shutdown.
	ctxt, cancel := p.CtxBlocking()
	defer cancel()

	headerVerifier := tapgarden.GenHeaderVerifier(ctxt, p.cfg.ChainBridge)
	for idx := range proofs {
		err := proof.ReplaceLastProofInBlob(
			ctxt, proofs[idx], p.cfg.ProofArchive, headerVerifier,
		)
		if err != nil {
			return fmt.Errorf("unable to update sent proofs: %w",
				err)
		}
	}
