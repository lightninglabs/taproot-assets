...
const (
	...
	// we expect a send fragment to be valid for.
	DefaultSendFragmentExpiryDelta = 210_000
)
...
func (p *ChainPorter) storeProofs(sendPkg *sendPackage) error {
	...
	for idx := range parcel.Outputs {
		...
		for outIdx := range sendPkg.SendManifests {
			...
			manifest.ExpiryHeight = parsedSuffix.BlockHeight +
				DefaultSendFragmentExpiryDelta
...
func (p *ChainPorter) stateStep(currentPkg sendPackage) (*sendPackage, error) {
	...
	case SendStateStorePostAnchorTxConf:
		...
		manifests, err := createSendFragments(currentPkg.VirtualPackets)
		if err != nil {
