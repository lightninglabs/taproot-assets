...
func (f *AssetWallet) FundAddressSend(ctx context.Context,
	...
	hasV2Addr := fn.Any(
		receiverAddrs, func(a *address.Tap) bool {
...
func prepareV2Outputs(ctx context.Context,
	...
	for pktIdx := range fundedVPkt.VPackets {
		...
		for outIdx := range vPkt.Outputs {
			...
			if vOut.Address.Version < address.V2 {
				continue
			}
...
// packets. The send fragments are used to reconstruct the information required
// to fetch proofs from the universe, and to materialize the asset outputs on
// the receiver's side. We assume that the receiver has access to the TAP
// address that was used to send the assets.
func createSendFragments(
	packets []*tappsbt.VPacket) (map[uint32]*proof.SendManifest, error) {

	result := make(map[uint32]*proof.SendManifest)
	addToFragment := func(addr *address.Tap, assetID asset.ID,
		vOut *tappsbt.VOutput) error {

		receiverKey := addr.ScriptKey

		manifest, ok := result[vOut.AnchorOutputIndex]
		if !ok {
			...
			result[vOut.AnchorOutputIndex] = manifest
		}
	...
	for _, vPkt := range packets {
		...
		for outIdx, vOut := range vPkt.Outputs {
			...
			err := addToFragment(vOut.Address, assetID, vOut)
			if err != nil {
