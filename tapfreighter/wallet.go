...
func (f *AssetWallet) AnchorVirtualTransactions(ctx context.Context,
	...
	// Add our anchor inputs first, so that BTC inputs exist when
	// AssertAnchorTimeLocks (called inside UpdateTaprootOutputKeys)
	// sets their sequences for relative timelocks.
	addAnchorPsbtInputs(sendPacket, params.ActivePackets)

	// Now update the PSBT packets to insert the _real_ outputs we
