...
//
// Privacy note: backup files contain key derivation paths (key family and
// index) for anchor internal keys and script keys. This is similar to
// exporting an xpub or a descriptor — anyone with access to the backup
// file can derive the public keys used for the wallet's on-chain outputs.
// Backup files should be treated as sensitive and stored securely.
package backup
