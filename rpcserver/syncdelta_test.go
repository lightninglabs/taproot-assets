...
// TestSyncDeltaTailReport tests the empty-page tail semantics: a
// caught-up caller reads back its own cursor, while a caller whose
// cursor lies beyond the journal reads back the committed tail — the
// signal it uses to detect a rewound or replaced journal.
func TestSyncDeltaTailReport(t *testing.T) {
	ctx := context.Background()
	h := newSyncDeltaHarness(t)

	// Locate the tail by consuming the full delta in one page.
	full, err := h.rpc.SyncDelta(ctx, &unirpc.SyncDeltaRequest{
		SinceSeq: 0,
		PageSize: 100,
	})
	...
	// Caught up: the response echoes the cursor.
	caughtUp, err := h.rpc.SyncDelta(ctx, &unirpc.SyncDeltaRequest{
		SinceSeq: tail,
	})
	require.NoError(t, err)
	require.Empty(t, caughtUp.Items)
	require.Equal(t, tail, caughtUp.LatestSeq)

	// Beyond the journal: the response reports the tail, strictly
	// below the requested cursor.
	beyond, err := h.rpc.SyncDelta(ctx, &unirpc.SyncDeltaRequest{
		SinceSeq: tail + 100,
	})
	require.NoError(t, err)
	require.Empty(t, beyond.Items)
	require.Equal(t, tail, beyond.LatestSeq)
}
