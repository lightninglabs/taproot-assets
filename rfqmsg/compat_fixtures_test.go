...
//
// Run with: make unit gen-test-vectors=true pkg=rfqmsg \
// case=^TestGenerateCompatFixtures$
func TestGenerateCompatFixtures(t *testing.T) {
	// Intentionally not parallel: this test writes fixtures into the
	// shared testdata/compat directory that TestBackwardCompatFixtures
	// reads, so it must run sequentially (before the parallel tests) to
	// avoid a read/write race on those files.
	...
	expiry := uint64(4102444800)

	accept := acceptWireMsgData{
		Version:      tlv.NewPrimitiveRecord[tlv.TlvType0](V1),
		ID:           tlv.NewPrimitiveRecord[tlv.TlvType2](id),
		Expiry:       tlv.NewPrimitiveRecord[tlv.TlvType4](expiry),
