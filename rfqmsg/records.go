...
func (r *RfqIDs) Record() tlv.Record {
	size := func() uint64 {
		var (
			buf     bytes.Buffer
			scratch [8]byte
		)
		err := encodeAvailableIDs(&buf, r, &scratch)
		if err != nil {
	...
	return tlv.MakeDynamicRecord(
		0, r, size, encodeAvailableIDs, decodeRfqIDs,
	)
}

// encodeAvailableIDs encodes the RfqIDs struct on the wire.
func encodeAvailableIDs(w io.Writer, val any, buf *[8]byte) error {
	if rfqIDs, ok := val.(*RfqIDs); ok {
		ids := rfqIDs.IDs
		num := uint64(len(ids))
...
func SomeRfqIDsRecord(
	ids []ID) tlv.OptionalRecordT[AvailableRfqIDsType, RfqIDs] {

	return tlv.SomeRecordT(tlv.NewRecordT[AvailableRfqIDsType, RfqIDs](
		RfqIDs{IDs: ids},
	))
}
...
type Htlc struct {
	...
	// bandwidth.
	AvailableRfqIDs tlv.OptionalRecordT[AvailableRfqIDsType, RfqIDs]
}
...
func (h *Htlc) Records() []tlv.Record {
	...
	h.AvailableRfqIDs.WhenSome(
		func(r tlv.RecordT[AvailableRfqIDsType, RfqIDs]) {
			records = append(records, r.Record())
		},
	)
...
func (h *Htlc) AsJson() ([]byte, error) {
	...
	h.AvailableRfqIDs.ValOpt().WhenSome(func(rfqIDs RfqIDs) {
