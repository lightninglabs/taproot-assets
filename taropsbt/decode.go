...
func NewFromPsbt(packet *psbt.Packet) (*VPacket, error) {
	...
	if len(packet.Inputs) != 1 {
		return nil, fmt.Errorf("expected exactly one input, got %d",
			len(packet.Inputs))
	}
	if len(packet.UnsignedTx.TxIn) != 1 {
		return nil, fmt.Errorf("expected exactly one unsigned input, "+
			"got %d", len(packet.UnsignedTx.TxIn))
	}
	...
	err = vPacket.Input.decode(packet.Inputs[0])
	if err != nil {
		return nil, fmt.Errorf("error decoding virtual input: %w", err)
	}
...
func (i *VInput) decode(pIn psbt.PInput) error {
	...
	mapping := []struct {
		...
		key:     PsbtKeyTypeInputTaroTapscriptSibling,
		decoder: tlvDecoder(&i.TapscriptSibling, tlv.DVarBytes),
	}, {
		key:     PsbtKeyTypeInputTaroAsset,
		decoder: assetDecoder(&i.asset),
	}, {
		key:     PsbtKeyTypeInputTaroSplitCommitment,
		decoder: booleanDecoder(&i.IsSplit),
	}}
...
func (o *VOutput) decode(pOut psbt.POutput, txOut *wire.TxOut) error {
	...
	mapping := []struct {
		...
		key:     PsbtKeyTypeOutputTaroIsChange,
		decoder: booleanDecoder(&o.IsChange),
	}, {
...
func value(unknowns []*psbt.Unknown, key []byte) ([]byte, error) {
	...
	return nil, fmt.Errorf("key %x not found in list of unkonwns", key)
}
