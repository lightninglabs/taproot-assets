...
func randomPacket(t testing.TB) *VPacket {
	...
	assetCommitment, err := commitment.NewAssetCommitment(testAsset)
	require.NoError(t, err)
	taroCommitment, err := commitment.NewTaroCommitment(assetCommitment)
	require.NoError(t, err)

	_, testProof, err := taroCommitment.Proof(
		testAsset.AssetCommitmentKey(),
		testAsset.TaroCommitmentKey(),
	)
	require.NoError(t, err)
	...
	vPacket.SetInputAsset(0, testAsset, testProof)
