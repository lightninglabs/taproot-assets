...
func (p *VPacket) EncodeAsPsbt() (*psbt.Packet, error) {
	pIn, txIn, err := p.Input.encode(p.ChainParams.HDCoinType)
	if err != nil {
...
func (i *VInput) encode(coinType uint32) (psbt.PInput, *wire.TxIn, error) {
	txIn := &wire.TxIn{}
	pIn := i.PInput
	...
	mapping := []struct {
		...
		encoder: tlvEncoder(&i.TapscriptSibling, tlv.EVarBytes),
	}, {
		key:     PsbtKeyTypeInputTaroAsset,
		encoder: assetEncoder(i.asset),
	}, {
		key:     PsbtKeyTypeInputTaroSplitCommitment,
		encoder: booleanEncoder(i.IsSplit),
	}}

	for idx := range mapping {
		value, err := mapping[idx].encoder()
		if err != nil {
			return pIn, nil, fmt.Errorf("error encoding input key "+
				"%x: %w", mapping[idx].key, err)
		}
...
func (o *VOutput) encode(coinType uint32) (psbt.POutput, *wire.TxOut, error) {
	...
	mapping := []struct {
		...
		encoder: booleanEncoder(o.IsChange),
	}, {
