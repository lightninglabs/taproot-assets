...
func (p *VPacket) EncodeAsPsbt() (*psbt.Packet, error) {
	...
	for idx := range p.Inputs {
		pIn, txIn, err := p.Inputs[idx].encode()
		if err != nil {
			return nil, fmt.Errorf("error encoding input %d: %w",
				idx, err)
		}

		unsignedTx.TxIn[idx] = txIn
...
func (i *VInput) encode() (psbt.PInput, *wire.TxIn, error) {
	txIn := &wire.TxIn{}
	pIn := i.PInput
	...
	mapping := []struct {
		...
		encoder: proofEncoder(i.assetProof),
	}}
...
func proofEncoder(p *commitment.Proof) func() ([]byte, error) {
	if p == nil {
		return func() ([]byte, error) {
			return nil, nil
		}
	...
	return tlvEncoder(p, commitment.ProofEncoder)
}
