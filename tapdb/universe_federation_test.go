...
func TestFederationSyncCursor(t *testing.T) {
	...
	// A server the federation has never heard of also reports cursor
	// zero rather than an error: the envoy may race a concurrent
	// server removal.
	unknown := universe.NewServerAddrFromStr("unknown-host:10029")
	cursor, err = fedDB.FetchSyncCursor(ctx, unknown)
	require.NoError(t, err)
	require.Zero(t, cursor)
}
