...
func addTuplesInternal(ctx context.Context, db BaseUniverseStore,
	...
	for _, tup := range tuples {
		...
		// and also the raw bytes of the ignore point.
...
func (it *IgnoreUniverseTree) Sum(ctx context.Context,
	...
	groupKey, err := spec.UnwrapGroupKeyOrErr()
	if err != nil {
		return lfn.Errf[lfn.Option[uint64]]("%w: %w", ErrMissingGroupKey, err)
	}
...
func (it *IgnoreUniverseTree) ListTuples(ctx context.Context,
	...
	groupKey, err := spec.UnwrapGroupKeyOrErr()
	if err != nil {
		return lfn.Errf[lfn.Option[universe.IgnoreTuples]](
...
func (it *IgnoreUniverseTree) QueryTuples(ctx context.Context,
	...
	groupKey, err := spec.UnwrapGroupKeyOrErr()
	if err != nil {
		return lfn.Errf[lfn.Option[[]universe.AuthenticatedIgnoreTuple]](
			"%w: %w", ErrMissingGroupKey, err,
		)
	}
	namespace := subTreeNamespace(groupKey, supplycommit.IgnoreTreeType)
