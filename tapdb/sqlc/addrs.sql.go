...
SELECT
    ...
    raw_script_keys.key_id, raw_script_keys.raw_key, raw_script_keys.key_family, raw_script_keys.key_index, -- noqa: RF02,AL03
...
JOIN internal_keys AS taproot_keys
    ON addrs.taproot_key_id = taproot_keys.key_id
WHERE
    addrs.creation_time >= $1
    AND addrs.creation_time <= $2
    AND (
        $3 = FALSE
        OR (coalesce(addrs.managed_from IS NULL, FALSE)) = $3
    )
...
WHERE
    addr_events.status >= $1
    AND addr_events.status <= $2
    AND coalesce($3, addrs.taproot_output_key)
    = addrs.taproot_output_key
