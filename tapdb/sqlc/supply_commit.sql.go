...
func (q *Queries) FetchSupplyUpdateEventsForBackfill(ctx context.Context, numLimit int32) ([]FetchSupplyUpdateEventsForBackfillRow, error) {
	rows, err := q.db.QueryContext(ctx, FetchSupplyUpdateEventsForBackfill, numLimit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const QueryEmptySupplyCommitTransitions = `-- name: QueryEmptySupplyCommitTransitions :many
...
type QueryEmptySupplyCommitTransitionsRow struct {
	TransitionID         int64
	StateMachineGroupKey []byte
}
...
// nothing.
func (q *Queries) QueryEmptySupplyCommitTransitions(ctx context.Context) ([]QueryEmptySupplyCommitTransitionsRow, error) {
	rows, err := q.db.QueryContext(ctx, QueryEmptySupplyCommitTransitions)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []QueryEmptySupplyCommitTransitionsRow
	for rows.Next() {
		var i QueryEmptySupplyCommitTransitionsRow
		if err := rows.Scan(&i.TransitionID, &i.StateMachineGroupKey); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
