...
-- a REFERENCE and NOT NULL, so we'll have to enforce non-nilness outside of the
-- DB.
ALTER TABLE asset_seedlings
ADD COLUMN script_key_id BIGINT REFERENCES script_keys (script_key_id);

-- For a group anchor, we derive the internal key for the future group key
-- early, to allow use of custom group witnesses.
ALTER TABLE asset_seedlings
ADD COLUMN group_internal_key_id BIGINT REFERENCES internal_keys (key_id);
