...
-- from a three-column UNIQUE constraint (minting_point, script_key_bytes,
-- leaf_node_namespace) to the original two-column constraint (minting_point,
-- script_key_bytes).
--
-- DOWNGRADE STRATEGY:
--
-- Since the enhanced constraint allowed multiple entries with the same
-- minting_point and script_key_bytes (differing only by leaf_node_namespace),
-- we need to be selective about which rows to keep when reverting to the more
...
-- 3. Selectively migrates data (keeping only one row per
--    minting_point/script_key_bytes pair).
...
CREATE UNIQUE INDEX universe_leaves_unique_minting_script
ON old_universe_leaves (minting_point, script_key_bytes);
...
-- For each (minting_point, script_key_bytes) group, we keep only the row with
-- the lowest ID.
...
FROM universe_leaves AS ul
INNER JOIN sub ON ul.id = sub.min_id;
