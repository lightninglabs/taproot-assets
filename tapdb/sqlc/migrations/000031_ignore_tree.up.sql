...
-- from a two-column constraint (minting_point, script_key_bytes) to a
-- three-column constraint (minting_point, script_key_bytes,
-- leaf_node_namespace).
--
-- PROBLEM STATEMENT:
--
-- In the current schema, the universe_leaves table enforces uniqueness based on
-- minting_point and script_key_bytes. This design assumes an asset belongs to
-- only one type of universe tree (transfer or issuance). However, with the
-- introduction of "ignore" and "burn" universe trees, the same asset might need
-- to exist in multiple trees simultaneously.
--
-- SOLUTION:
--
-- We expand the unique constraint to include the leaf_node_namespace column.
-- This additional dimension allows distinguishing between assets based on which
-- universe tree they belong to, while still preventing duplicates within the
-- same tree.
--
-- MIGRATION STRATEGY:
--
-- Since SQLite has limited ALTER TABLE capabilities and we need to maintain
-- compatibility with both SQLite and PostgreSQL, we use a table recreation
-- approach:
--
-- 1. Handle foreign key dependencies (back up and remove
...
--
-- This approach works with both database engines while preserving data
-- integrity.
--

-- ==== PHASE 1: HANDLE FOREIGN KEY DEPENDENCIES ====
-- Before we can drop the universe_leaves table, we need to temporarily remove
-- any foreign key references pointing to it. The federation_proof_sync_log
-- table has a foreign key to universe_leaves.id that would prevent dropping the
-- table.
...
CREATE TABLE new_federation_proof_sync_log (
    ...
    -- FK constraint intentionally omitted for now.
...
DROP INDEX
IF EXISTS federation_proof_sync_log_unique_index_proof_leaf_id_servers_id;

-- ==== PHASE 2: CREATE NEW TABLE WITH UPDATED CONSTRAINT ====
-- Create a new universe_leaves table with the enhanced 3-column unique
-- constraint.
...
ON new_universe_leaves (minting_point, script_key_bytes, leaf_node_namespace);
...
CREATE INDEX IF NOT EXISTS universe_leaves_key_idx ON universe_leaves (
    leaf_node_key
);
CREATE INDEX IF NOT EXISTS universe_leaves_namespace ON universe_leaves (
    leaf_node_namespace
);

-- ==== PHASE 5: RESTORE DEPENDENT TABLES ====
-- Recreate the federation_proof_sync_log table with proper foreign key
-- references.
...
-- Only reinsert records that reference existing leaves in the universe_leaves
-- table.
INSERT INTO federation_proof_sync_log
SELECT * FROM new_federation_proof_sync_log
WHERE proof_leaf_id IN (SELECT ul.id FROM universe_leaves AS ul);
