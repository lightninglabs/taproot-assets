-- *****************************************************************************
-- UP MIGRATION: Expanding Allowed Proof Types with an Enum-Style Reference
-- Column
--
-- In this migration, we expand the allowed values for the "proof_type" column
-- in our universe-related tables by introducing a new enum-like table
-- ("proof_types").
--
-- Changes introduced:
...
--         a) Add a new column "proof_type_ext" with a NOT NULL constraint and a
--            foreign key reference to proof_types(proof_type).
--         b) Copy the data from the existing "proof_type" column into
--            "proof_type_ext".
--         c) Rename the original "proof_type" to "proof_type_old".
--         d) Rename "proof_type_ext" to "proof_type" so that the final column
--            name remains unchanged.
--         e) Drop the temporary "proof_type_old" column.
--
-- This approach preserves existing data while allowing new rows to use the
-- expanded set of proof types. 
-- *****************************************************************************
...
-- Create a new table "proof_types" to hold the allowed values for the proof
-- type field. Insert the allowed values.
--------------------------------------------------------------------------------
...
-- Since the proof_type column here is a primary key, we cannot drop it
-- directly. Therefore, we rename the existing table, create a new table with
-- the updated schema, copy the data over, and drop the old table.
--------------------------------------------------------------------------------
ALTER TABLE federation_global_sync_config
RENAME TO federation_global_sync_config_old;
...
-- Add a new column ("proof_type_ext"), copy old data, then rename columns to
-- finalize changes.
--------------------------------------------------------------------------------
...
ALTER TABLE federation_uni_sync_config
RENAME COLUMN proof_type TO proof_type_old;
ALTER TABLE federation_uni_sync_config
RENAME COLUMN proof_type_ext TO proof_type;
ALTER TABLE federation_uni_sync_config DROP COLUMN proof_type_old;

--------------------------------------------------------------------------------
-- Section 6: Re-create the universe_stats View 
-- Rebuild the view using the latest definition (from
-- 000027_better_universe_stats.up.sql) so that downstream queries see the
-- updated schema for universe_roots.
--------------------------------------------------------------------------------
