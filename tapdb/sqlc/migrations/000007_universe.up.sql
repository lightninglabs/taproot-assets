...
CREATE INDEX IF NOT EXISTS universe_roots_asset_id_idx ON universe_roots (
    asset_id
);
CREATE INDEX IF NOT EXISTS universe_roots_group_key_idx ON universe_roots (
    group_key
);
...
CREATE INDEX IF NOT EXISTS universe_servers_host ON universe_servers (
    server_host
);
...
CREATE INDEX IF NOT EXISTS universe_events_event_time_idx ON universe_events (
    event_time
);
CREATE INDEX IF NOT EXISTS universe_events_type_idx ON universe_events (
    event_type
);
...
INNER JOIN universe_roots AS roots ON u.universe_root_id = roots.id
...
CREATE TABLE IF NOT EXISTS federation_global_sync_config (
    -- This field is an enum representing the proof type stored in the given
    -- universe.
    proof_type TEXT NOT NULL PRIMARY KEY CHECK (
        proof_type IN ('issuance', 'transfer')
    ),
