...
--
-- Changes:
-- 1. For federation_global_sync_config, re-create the table using the original
--    schema since the proof_type column is a primary key. This avoids dropping
--    a primary key column.
-- 2. For other tables, revert changes via ALTER statements.
-- 3. Drop the 'proof_types' enum table.
-- 4. Recreate the 'universe_stats' view using the latest definition to match
--    the reverted schema.
-- *****************************************************************************

-- For universe_roots
ALTER TABLE universe_roots ADD COLUMN proof_type TEXT NOT NULL CHECK (
...
RENAME TO federation_global_sync_config_new;
...
RENAME COLUMN proof_type TO proof_type_new;
ALTER TABLE federation_uni_sync_config
ADD COLUMN proof_type TEXT NOT NULL CHECK (
    proof_type IN ('issuance', 'transfer')
);
