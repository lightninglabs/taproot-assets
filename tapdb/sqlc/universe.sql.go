...
WHERE
    r.proof_type = $1
    AND (l.asset_id = $2 OR $2 IS NULL)
    AND (l.group_key = $3 OR $3 IS NULL)
`
...
JOIN genesis_info_view AS gen
    ON leaves.asset_genesis_id = gen.gen_asset_id
WHERE
    leaves.leaf_node_namespace = $1
    AND (
        leaves.minting_point = $2
        OR $2 IS NULL
    )
    AND (
        leaves.script_key_bytes = $3
        OR $3 IS NULL
    )
`
