...
SELECT
    ...
    universe_roots.group_key,
    universe_roots.proof_type,
    mssmt_nodes.hash_key AS root_hash,
    mssmt_nodes.sum AS root_sum,
...
JOIN mssmt_nodes
    ON
        mssmt_roots.root_hash = mssmt_nodes.hash_key
        AND mssmt_roots.namespace = mssmt_nodes.namespace
JOIN genesis_assets
    ON universe_roots.asset_id = genesis_assets.asset_id
...
JOIN mssmt_nodes AS nodes
    ON
        leaves.leaf_node_key = nodes.key
        AND leaves.leaf_node_namespace = nodes.namespace
JOIN genesis_info_view AS gen
    ON leaves.asset_genesis_id = gen.gen_asset_id
WHERE
    leaves.leaf_node_namespace = @namespace
    AND (
        leaves.minting_point = sqlc.narg('minting_point_bytes')
        OR sqlc.narg('minting_point_bytes') IS NULL
    )
    AND (
        leaves.script_key_bytes = sqlc.narg('script_key_bytes')
        OR sqlc.narg('script_key_bytes') IS NULL
    );

-- name: FetchUniverseKeys :many
SELECT
    leaves.minting_point,
    leaves.script_key_bytes
FROM universe_leaves AS leaves
...
WITH group_key_root_id AS (
    SELECT roots.id
    FROM universe_roots AS roots
    WHERE
        roots.group_key = @group_key_x_only
...
INSERT INTO universe_events (
    ...
    'SYNC',
    CASE
        WHEN length(@group_key_x_only) > 0 THEN (
...
asset_info AS (
    SELECT
        ...
        gen.anchor_txid
    FROM genesis_info_view AS gen
    JOIN asset_supply
        ON gen.asset_id = asset_supply.asset_id
    ...
    LEFT JOIN key_group_info_view AS group_info
        ON gen.gen_asset_id = group_info.gen_asset_id
    LEFT JOIN group_supply
        ON group_info.x_only_group_key = group_supply.group_key
    WHERE (
        gen.asset_tag = sqlc.narg('asset_name')
        OR sqlc.narg('asset_name') IS NULL
    )
    AND (
        gen.asset_type = sqlc.narg('asset_type')
        OR sqlc.narg('asset_type') IS NULL
    )
    AND (gen.asset_id = sqlc.narg('asset_id') OR sqlc.narg('asset_id') IS NULL)
)
...
ORDER BY
    CASE
        WHEN
            sqlc.narg('sort_by') = 'asset_id'
            AND sqlc.narg('sort_direction') = 0
            THEN
                asset_info.asset_id
    END ASC,
    CASE
        WHEN
            sqlc.narg('sort_by') = 'asset_id'
            AND sqlc.narg('sort_direction') = 1
            THEN
                asset_info.asset_id
    END DESC,
    CASE
        WHEN
            sqlc.narg('sort_by') = 'asset_name'
            AND sqlc.narg('sort_direction') = 0
            THEN
                asset_info.asset_name
    END ASC,
    ...
    CASE
        WHEN
            sqlc.narg('sort_by') = 'total_syncs'
            AND sqlc.narg('sort_direction') = 0
            THEN
                universe_stats.total_asset_syncs
    END ASC,
    CASE
        WHEN
            sqlc.narg('sort_by') = 'total_syncs'
    ...
    CASE
        WHEN
            sqlc.narg('sort_by') = 'total_proofs'
            AND sqlc.narg('sort_direction') = 0
            THEN
                universe_stats.total_asset_proofs
    END ASC,
    CASE
        WHEN
            sqlc.narg('sort_by') = 'total_proofs'
    ...
    CASE
        WHEN
            sqlc.narg('sort_by') = 'genesis_height'
            AND sqlc.narg('sort_direction') = 0
            THEN
                asset_info.genesis_height
    END ASC,
    ...
    CASE
        WHEN
            sqlc.narg('sort_by') = 'total_supply'
            AND sqlc.narg('sort_direction') = 0
            THEN
                asset_info.supply
    END ASC,
...
SELECT
    cast(strftime('%Y-%m-%d', datetime(event_timestamp, 'unixepoch')) AS text)
        AS day,
    sum(CASE WHEN event_type = 'SYNC' THEN 1 ELSE 0 END) AS sync_events,
    sum(CASE WHEN event_type = 'NEW_PROOF' THEN 1 ELSE 0 END)
        AS new_proof_events
FROM universe_events
WHERE
    event_type IN ('SYNC', 'NEW_PROOF')
    AND event_timestamp >= @start_time AND event_timestamp <= @end_time
GROUP BY day
ORDER BY day;
...
INSERT INTO federation_proof_sync_log (
    ...
    (
        ...
        WHERE
            leaf_node_namespace = @leaf_namespace
...
attempt_counter = CASE
    WHEN @bump_sync_attempt_counter = TRUE
        THEN federation_proof_sync_log.attempt_counter + 1
    ELSE federation_proof_sync_log.attempt_counter
END
RETURNING id;
...
JOIN universe_servers AS server
    ON log.servers_id = server.id
JOIN universe_roots AS root
    ON log.universe_root_id = root.id
WHERE (
    log.sync_direction = sqlc.narg('sync_direction')
...
WHERE
    servers_id IN (SELECT id FROM selected_server_id)
    AND (
    ...
    AND (
        ...
        OR sqlc.narg('min_timestamp') IS NULL
    )
    AND (
...
SELECT
    ...
    n.sum AS multiverse_root_sum
FROM multiverse_roots AS r
JOIN mssmt_roots AS m
    ON r.namespace_root = m.namespace
JOIN mssmt_nodes AS n
    ON
        m.root_hash = n.hash_key
        AND m.namespace = n.namespace
...
SELECT
    ...
    l.asset_id,
    l.group_key,
    smt_nodes.value AS universe_root_hash,
    smt_nodes.sum AS universe_root_sum
FROM multiverse_leaves AS l
JOIN mssmt_nodes AS smt_nodes
    ON
        l.leaf_node_key = smt_nodes.key
        AND l.leaf_node_namespace = smt_nodes.namespace
JOIN multiverse_roots AS r
    ON l.multiverse_root_id = r.id
WHERE
    r.proof_type = @proof_type
    AND (l.asset_id = @asset_id OR @asset_id IS NULL)
    AND (l.group_key = @group_key OR @group_key IS NULL);
