...
func (s *SupplyTreeStore) FetchSubTrees(ctx context.Context,
	...
	groupKey, err := spec.UnwrapGroupKeyOrErr()
	if err != nil {
		return lfn.Errf[supplycommit.SupplyTrees]("group key must be specified for supply tree: %w", err)
	}
...
func (s *SupplyTreeStore) FetchRootSupplyTree(ctx context.Context,
	...
	groupKey, err := spec.UnwrapGroupKeyOrErr()
	if err != nil {
		return lfn.Errf[mssmt.Tree]("group key must be specified for supply tree: %w", err)
	}
...
func applySupplyUpdatesInternal(ctx context.Context, dbTx BaseUniverseStore,
	...
	groupKey, err := spec.UnwrapGroupKeyOrErr()
	if err != nil {
		return nil, fmt.Errorf("group key must be specified for supply updates: %w", err)
	}
