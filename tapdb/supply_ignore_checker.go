...
// possibility that the list of ignored asset points changes, such as when a new
// ignore tuple is added to the asset group.
func (c *CachingIgnoreChecker) InvalidateCache() {
	c.Lock()
	defer c.Unlock()

	log.Debugf("Invalidating ignore checker negative lookup cache")

	c.nonIgnoredAssetPoints = newNonIgnoredCache(
		c.cfg.NegativeLookupCacheSize,
	)
}
