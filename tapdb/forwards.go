...
// QueryForwards retrieves forwarding event records matching the given filters.
func (s *PersistedForwardStore) QueryForwards(ctx context.Context,
	params rfq.QueryForwardsParams) ([]rfq.ForwardingEvent, error) {
	...
	readOpts := ReadTxOption()
	var records []rfq.ForwardingEvent

	err := s.db.ExecTx(ctx, readOpts, func(q ForwardStore) error {
		rows, err := q.QueryForwards(ctx, queryParams)
		if err != nil {
			return fmt.Errorf("error querying forwarding events: "+
				"%w", err)
		}
...
func (s *PersistedForwardStore) CountForwards(ctx context.Context,
	...
	err := s.db.ExecTx(ctx, readOpts, func(q ForwardStore) error {
		...
		if err != nil {
			return fmt.Errorf("error counting forwarding events: "+
				"%w", err)
		}
