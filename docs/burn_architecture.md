...
### Full Burn Prevention

The system prevents burning all assets in an anchor output, a restriction that
can create problematic scenarios:

```go
ErrFullBurnNotSupported = errors.New("burning all assets of an anchor output is not supported")
```

This limitation exists to prevent the creation of empty anchor outputs that
would waste Bitcoin UTXO space without containing any useful asset commitments.
However, the restriction can be overly conservative in practice. Users may find
themselves with assets that become effectively unburnable if they're the last
assets in an output, forcing them to maintain "dust" assets indefinitely.

The implementation checks whether other assets exist in the anchor output and
blocks the burn if none are found. While this prevents UTXO waste, it doesn't
account for scenarios where users might want to clean up their UTXO set by
burning remaining assets and consolidating their holdings. A more nuanced
approach might allow full burns when combined with UTXO consolidation
operations.
