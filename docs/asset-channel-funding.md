...
solution to a complex problem: how do you lnd, and the BOLT protocol to support
Taproot Assets, without embedding all the core logic within `lnd` itself? The
answer lies in LND's prescient `AuxFundingController` interface, which provides
clean extension points for custom channel types while preserving Lightning
Network's security and operational properties.
...
`msgmax` and custom message features to insert new messages into the funding
