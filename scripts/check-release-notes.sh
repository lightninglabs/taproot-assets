...
# actually be "main" here, so we'll ignore this case, and assume that in
...
if [[ $PR_NUMBER == "main" ]] || [[ $PR_NUMBER == "gh-readonly-queue" ]]; then
    exit 0
fi

# Ensure that the PR number at least shows up in the release notes folder under
# one of the contained milestones.
if ! grep -r -q "lightninglabs/taproot-assets/pull/$PR_NUMBER" docs/release-notes; then
