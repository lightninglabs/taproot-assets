...
func lastProofFromBlobWithHint(blob Blob) (*Proof, []byte, *blobAppendHint,
	...
	if !IsProofFile(blob) {
		return nil, nil, nil, fmt.Errorf(
			"blob is not a valid proof file",
		)
	}
	...
	for i := uint64(0); i < count; i++ {
		...
		if i < count-1 {
			// Skip proof bytes and read the stored chained hash for
			// all but the last proof.
			if _, err := r.Seek(
				int64(numProofBytes), io.SeekCurrent,
			); err != nil {
				return nil, nil, nil, fmt.Errorf(
					"skipping proof bytes (idx=%d): %w",
					i, err,
				)
			}
