...
func TestProofEncodingGroupKeyRevealV1(t *testing.T) {
	...
	for _, version := range versions {
		...
		randProof.GroupKeyReveal = &groupKeyReveal

		file, err := proof.NewFile(proof.V0, randProof, randProof)
		require.NoError(t, err)
		randProof.AdditionalInputs = []proof.File{*file, *file}
		...
		assertEqualProof(t, &randProof, &decodedProof)
	}
}
...
func TestProofEncoding(t *testing.T) {
	...
	proof1.GroupKeyReveal.SetTapscriptRoot(nil)
	file, err = proof.NewFile(proof.V0, proof1, proof1)
	require.NoError(t, err)
	proof1.AdditionalInputs = []proof.File{*file, *file}
	...
	for _, tc := range testCases {
		...
		t.Run(tc.name, func(tt *testing.T) {
			...
			_, err := genesisProof.Verify(
				context.Background(), nil, proof.MockChainLookup,
...
func TestProofBlockHeaderVerification(t *testing.T) {
	...
	_, err := p.Verify(
		context.Background(), nil, proof.MockChainLookup, vCtx,
	)
	require.NoError(t, err)
...
func TestProofFileVerification(t *testing.T) {
	...
	_, err = f.Verify(context.Background(), proof.MockVerifierCtx)
	require.NoError(t, err)

	// Ensure that verification of a proof of unknown version fails.
	f.Version = proof.Version(212)

	lastAsset, err := f.Verify(context.Background(), proof.MockVerifierCtx)
...
func TestProofFileVerificationIgnoreChecker(t *testing.T) {
	...
	rapid.Check(t, func(t *rapid.T) {
		...
		if reject {
			...
			ignoreChecker := proof.NewMockIgnoreChecker(
				false, assetPoint,
			)
			vCtx.IgnoreChecker = lfn.Some[proof.IgnoreChecker](
				ignoreChecker,
			)
		}
...
// TLV types.
func checkTaprootProofTypes(t *testing.T, p proof.TaprootProof) {
	for _, record := range p.EncodeRecords() {
		...
		require.Contains(t, proof.KnownTaprootProofTypes, record.Type())
	}
	...
	if p.CommitmentProof != nil {
		for _, record := range p.CommitmentProof.EncodeRecords() {
			...
			require.Contains(
				t, proof.KnownCommitmentProofTypes,
				record.Type(),
			)
	...
	if p.TapscriptProof != nil {
		for _, record := range p.TapscriptProof.EncodeRecords() {
			...
			require.Contains(
				t, proof.KnownTapscriptProofTypes,
				record.Type(),
			)
		}
	}
...
func TestProofUnknownOddType(t *testing.T) {
	...
	genesis := asset.RandGenesis(t, asset.Collectible)
	scriptKey := test.RandPubKey(t)
	knownProof := proof.RandProof(t, genesis, scriptKey, oddTxBlock, 0, 1)

	var knownProofBytes []byte
	test.RunUnknownOddTypeTest(
		t, &knownProof, &asset.ErrUnknownType{},
		func(buf *bytes.Buffer, proof *proof.Proof) error {
		...
		func(parsedProof *proof.Proof, unknownTypes tlv.TypeMap) {
			require.Equal(
...
func init() {
	...
	proof.UseLogger(logger.SubSystem(proof.Subsystem))
}
