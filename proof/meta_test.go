	...
	testCases := []struct {
		...
		},
		{
			name: "new asset meta reveal with explicit zero " +
				"decimal display and URL",
			reveal: &MetaReveal{
				Type:              MetaOpaque,
				Data:              []byte(`not JSON`),
				DecimalDisplay:    fn.Some[uint32](0),
				CanonicalUniverse: fn.MaybeSome(dummyURL),
			},
			expectedErr: nil,
		},
		{
			...
			reveal: &MetaReveal{
				Type:              MetaOpaque,
				Data:              []byte(`not JSON`),
				CanonicalUniverse: fn.MaybeSome(tooLongURL),
			},
			expectedErr: ErrCanonicalUniverseTooLong,
		},
	}
	...
	testCases := []struct {
		...
		},
		{
			...
			expected: []byte{
				...
				// CanonicalUniverse:
				0x07, 0x1c, 0x75, 0x6e, 0x69, 0x76, 0x65, 0x72,
				0x73, 0x65, 0x72, 0x70, 0x63, 0x3a, 0x2f, 0x2f,
				0x6c, 0x6f, 0x63, 0x61, 0x6c, 0x68, 0x6f, 0x73,
				0x74, 0x3a, 0x31, 0x32, 0x33, 0x34,
			},
		},
		{
			name: "new asset meta reveal with explicit zero " +
...
func TestDecodeOldMetaReveal(t *testing.T) {
	...
	require.Equal(t, fn.None[url.URL](), decoded.CanonicalUniverse)
}
