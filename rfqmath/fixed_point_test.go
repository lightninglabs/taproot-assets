...
func testCmpScaleInvariant[N Int[N]](t *rapid.T) {
	...
	// Create two fixed points representing the same value at different
	// scales.
	fp1 := FixedPointFromUint64[N](coefficient, scale1)
	fp2 := FixedPointFromUint64[N](coefficient, scale2)
...
func testCasesCmp[N Int[N]](t *testing.T) {
	...
	testCases := []testCase{
		...
		{
			name:     "first less than second different scales",
