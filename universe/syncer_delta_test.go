...
func (m *memUniverseSet) SyncDelta(_ context.Context, sinceSeq uint64,
	...
	// Mirror the production tail semantics: an empty page reports the
	// committed journal tail, which sits below sinceSeq when the
	// caller's cursor lies beyond this journal.
	if len(page.Items) == 0 {
		page.LatestSeq = 0
		if len(m.journal) > 0 {
			page.LatestSeq = m.journal[len(m.journal)-1].Seq
		}
	}
...
// stubDeltaEngine serves scripted latest_seq values with empty pages,
// modeling a remote that violates the paging protocol.
type stubDeltaEngine struct {
	*memUniverseSet

	latest func(sinceSeq uint64) uint64
}

func (s *stubDeltaEngine) SyncDelta(_ context.Context, sinceSeq uint64,
	_ int32) (*DeltaPage, error) {

	return &DeltaPage{
		Roots:     make(map[IdentifierKey]Root),
		LatestSeq: s.latest(sinceSeq),
	}, nil
}

// TestSyncUniverseDeltaJournalRewind tests that a cursor beyond the
// remote's journal tail is detected and surfaced as ErrJournalRewind
// rather than silently reported as an empty, successful delta.
func TestSyncUniverseDeltaJournalRewind(t *testing.T) {
	t.Parallel()
	...
	for i := 0; i < 3; i++ {
		require.NoError(t, remote.insert(
			id, randomTestLeafKey(t), randomTestLeaf(t),
		))
	}
	...
	tail := remote.maxSeq()
	cursor := tail + 5
	_, err := syncer.SyncUniverseDelta(ctx, addr, cursor, allowAllConfigs())

	var rewindErr *ErrJournalRewind
	require.ErrorAs(t, err, &rewindErr)
	require.Equal(t, cursor, rewindErr.Cursor)
	require.Equal(t, tail, rewindErr.Tail)
}

// TestSyncUniverseDeltaNonMonotoneRemote tests that a remote whose
// latest_seq answers regress mid-run terminates the paging loop with
// the rewind error instead of cycling indefinitely.
func TestSyncUniverseDeltaNonMonotoneRemote(t *testing.T) {
	t.Parallel()

	ctx := context.Background()

	// Flip-flop remote: a low cursor is answered with 100, a high one
	// with 5. Unguarded, these answers alternate forever.
	stub := &stubDeltaEngine{
		memUniverseSet: newMemUniverseSet(),
		latest: func(sinceSeq uint64) uint64 {
			if sinceSeq < 100 {
				return 100
			}
	...
	local := newMemUniverseSet()
	syncer := newMemSyncer(
		local, func() DiffEngine { return stub }, 50,
	)
	addr := NewServerAddrFromStr("flip-flop-peer:10029")

	_, err := syncer.SyncUniverseDelta(ctx, addr, 0, allowAllConfigs())

	var rewindErr *ErrJournalRewind
	require.ErrorAs(t, err, &rewindErr)
	require.Equal(t, uint64(100), rewindErr.Cursor)
	require.Equal(t, uint64(5), rewindErr.Tail)
}

// TestSyncUniverseDeltaPageCap tests that a remote advancing the
// cursor indefinitely without ever converging is cut off by the page
// bound rather than holding the sync (and its global lock) forever.
func TestSyncUniverseDeltaPageCap(t *testing.T) {
	t.Parallel()
	...
	addr := NewServerAddrFromStr("endless-peer:10029")

	_, err := syncer.SyncUniverseDelta(ctx, addr, 0, allowAllConfigs())
	require.ErrorContains(t, err, "pages")
}
