...
func (p *Publisher) PublishMintBatch(ctx context.Context,
	...
	for start := uint(0); start < numTotal; start += p.batchSize {
		...
		err := p.reg.UpsertProofLeafBatch(ctx, chunk)
		switch {
		// The leaves are durably stored; only their multiverse
		// entries are still outstanding, and the universe
		// archive repairs those in the background. The batch
		// must not be failed over writes that landed.
