...
func (a *GenesisAugmenter) BindData(_ context.Context,
	...
	if batch.GenesisPacket == nil {
		return zero, fmt.Errorf("supply-commit batch has no funded " +
			"genesis packet")
	}
	...
	if dKey.IsNone() {
		return zero, fmt.Errorf("supply-commit batch has no " +
			"delegation key")
	}
	...
	if outIdx.IsNone() {
		return zero, fmt.Errorf("pre-commit output not found in " +
			"funded psbt")
	}
...
func delegationKeyFromBatch(batch *tapgarden.MintingBatch) (
	...
	delegationKey, err := anchor.DelegationKey.UnwrapOrErr(
		fmt.Errorf("delegation key unexpectedly absent"),
	)
	if err != nil {
		return zero, err
	...
	return fn.Some(delegationKey), nil
}
...
func findPreCommitOutputIdx(funded *tapsend.FundedPsbt,
	...
	if funded == nil {
		return zero, fmt.Errorf("funded psbt is nil")
	}
	if funded.Pkt == nil {
		return zero, fmt.Errorf("funded psbt packet is nil")
	}
	...
	if tx == nil {
		return zero, fmt.Errorf("funded psbt has no unsigned " +
			"transaction")
	}
