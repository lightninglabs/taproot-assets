...
// taken from the anchoring's enriched witness. Two effects ride the
// transactional outbox behind the same commit: a nudge, so the
// resting state machine re-derives its position from the record the
// finalizer wrote, and the remote-universe push. The nudge is its own
// effect rather than a step of the push because the push is retried
// until every canonical universe holds the commitment, and a tick on
// every retry would commit whatever updates the operator has since
// queued, on chain, without the publish call that normally starts a
// cycle.
...
// are excluded. The heights serve as spend-subscription hints; an
// output whose confirmation height the record does not carry — a
// pre-commitment from a batch still awaiting confirmation — is
// registered without one, and the watcher hints it from the
// anchoring's registration height rather than the site guessing a
// height so low that the notifier would rescan the chain from there.
