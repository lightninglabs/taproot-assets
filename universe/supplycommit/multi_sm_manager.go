...
type MultiStateMachineManager struct {
	...
	// smCtx is the context used to manage the lifecycle of the state
	// machines.
...
func (m *MultiStateMachineManager) Start(ctx context.Context) {
	m.startOnce.Do(func() {
		// Initialize the state machine cache.
		m.smCache = NewStateMachineCache()

		// Create a new context for managing the lifecycle of the state
		// machines.
		m.smCtx, m.smCancel = context.WithCancel(ctx)
	})
}
...
func (m *MultiStateMachineManager) fetchStateMachine(
	...
	initialState, _, err := m.cfg.StateLog.FetchState(
		ctx, assetSpec,
	)
	if err != nil {
	...
	case *CommitBroadcastState:
		...
		// CommitFinalizeState. If we crashed right after that, then we'll also
		// send the finalize event so we can apply everything, and transition
		// back to the normal default state.
	...
	// context here which will be cancelled when the manager is stopped.
	newSm.Start(m.smCtx)
