...
func TestSupplyCommitRestart(t *testing.T) {
	...
	// place, re-publishes the persisted transaction — the record is
	// written before the publish, so a crash between the two or a
	// rejected publish must not strand it — and then rests: the
	// restart tick re-reads a record that still says broadcast, and
	...
	t.Run("anchoring_broadcast_rests", func(t *testing.T) {
		...
		// Looked up once on restart and once by the broadcast
		// handler's safety net; present both times, so nothing is
		// registered.
		registrar.On(
			"LookupByMatchKey", mock.Anything, SupplySiteID,
			mock.Anything,
		).Return(&tapreorg.Anchoring{ID: 1}, nil).Twice()
		...
		registrar.AssertExpectations(t)
		h.mockDaemon.AssertExpectations(t)
	})
	...
	// nothing since the state is already durable, re-publishes the
	// transaction and then rests.
	t.Run("anchoring_broadcast_adopts_missing", func(t *testing.T) {
		...
		// The broadcast handler's safety net finds the adopted
		// anchoring and registers nothing more.
		registrar.On(
			"LookupByMatchKey", mock.Anything, SupplySiteID,
			mock.Anything,
		).Return(&tapreorg.Anchoring{ID: 1}, nil).Once()
