...
// asserts it is of type S (which must be a pointer to a struct implementing State),
// and returns it.
func assertAndGetCurrentState[S State](h *supplyCommitTestHarness) S {
	...
	if !ok {
		...
		h.t.Fatalf("current state is of type %T (%s), but expected %T",
			currentStateUntyped, currentStateUntyped.String(), expectedType)
	}
...
func TestSupplyUpdateEventTypes(t *testing.T) {
	...
	t.Run("new_ignore_event", func(t *testing.T) {
		...
		require.Equal(t, IgnoreTreeType, ignoreEvent.SupplySubTreeType())
	...
	t.Run("new_mint_event_encode_decode", func(t *testing.T) {
		// To begin, we generate the necessary components to create a random
		// proof using proof.RandProof. This includes a script key and a dummy
		...
		originalMintEvent.IssuanceProof.GenesisWithGroup = universe.GenesisWithGroup{
		...
		require.True(t, ok, "LeafKey should be of type universe.AssetLeafKey")
		...
		originalMintEvent.eventSealed()
		require.Equal(t, MintTreeType, originalMintEvent.SupplySubTreeType())
...
func TestTxInMethods(t *testing.T) {
	...
	t.Run("pre_commitment_tx_in", func(t *testing.T) {
		...
		preCommit := PreCommitment{
			...
			PreCommitmentOutput: tapgarden.PreCommitmentOutput{
				OutIdx: 0,
			},
		}
...
func TestCreateCommitmentTxLabel(t *testing.T) {
	...
	t.Run("with_valid_supply_root", func(t *testing.T) {
		...
		require.Contains(t, label, "supply_commit")
