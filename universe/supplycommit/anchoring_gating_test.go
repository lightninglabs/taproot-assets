...
func TestSupplySiteActGating(t *testing.T) {
	...
	// exactly two effects: a nudge addressed to the group, so the
	// resting machine re-derives its position once, and the push,
	// carrying the anchoring's own payload so the dispatcher can
	// rebuild from durable state. The nudge is not the push's to
	// repeat: the push is retried until every universe holds the
	// commitment, and a tick per retry would start commit cycles
	...
	burialNudge := tx.effects[0]
	require.Equal(t, CommitNudgeEffectKind, burialNudge.Kind)
	require.Equal(t, anchoring.ID, burialNudge.Anchoring.UnwrapOr(0))
	nudged, err := decodeNudgeBlob(burialNudge.Payload)
	require.NoError(t, err)
	require.Equal(t, blob.GroupKey, nudged.GroupKey)

	require.Equal(t, CommitPushEffectKind, tx.effects[1].Kind)
	require.Equal(t, anchoring.ID, tx.effects[1].Anchoring.UnwrapOr(0))
	require.Equal(t, payload, tx.effects[1].Payload)
