...
func (d *DefaultState) ProcessEvent(event Event,
	...
	case *InitEvent:
		...
		lastCommit, err := env.SupplyCommitView.LastVerifiedCommitment(
			ctx, env.AssetSpec,
		)
		if err != nil {
...
func (w *WatchOutputsSpendState) ProcessEvent(event Event,
	...
	case *WatchLatestOutputsEvent:
		...
		supplyCommit, err := env.SupplyCommitView.SupplyCommit(
			ctx, env.AssetSpec,
		)
		if err != nil {
...
func (s *SyncSupplyProofsState) ProcessEvent(event Event,
	...
	case *SpendEvent:
		...
		ctx := context.Background()
		nextCommitment, err := env.SupplyCommitView.SupplyCommit(
			ctx, env.AssetSpec,
		)
		if err != nil {
