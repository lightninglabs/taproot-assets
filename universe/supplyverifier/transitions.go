...
func (d *DefaultState) ProcessEvent(event Event,
	...
	case *InitEvent:
		...
		lastCommitOpt, err := env.SupplyCommitView.SupplyCommit(
			ctx, env.AssetSpec,
		).Unpack()
		if err != nil {
		...
		lastCommit, err := lastCommitOpt.UnwrapOrErr(
			fmt.Errorf("no last verified commitment found"),
		)
		if err != nil {
			return nil, err
		}
...
func (w *WatchOutputsSpendState) ProcessEvent(event Event,
	...
	case *WatchLatestOutputsEvent:
		...
		supplyCommitOpt, err := env.SupplyCommitView.SupplyCommit(
			ctx, env.AssetSpec,
		).Unpack()
		if err != nil {
		...
		// TODO(ffranr): Handle case where no supply commitment found.
		supplyCommit, err := supplyCommitOpt.UnwrapOrErr(
			fmt.Errorf("no supply commitment found"),
		)
		if err != nil {
			return nil, err
		}
...
func (s *SyncSupplyProofsState) ProcessEvent(event Event,
	...
	case *SpendEvent:
		...
		ctx := context.Background()
		nextCommitmentOpt, err := env.SupplyCommitView.SupplyCommit(
			ctx, env.AssetSpec,
		).Unpack()
		if err != nil {
		...
		// TODO(ffranr): Handle case where no next commitment is found.
		nextCommitment, err := nextCommitmentOpt.UnwrapOrErr(
			fmt.Errorf("no next commitment found"),
		)
		if err != nil {
			return nil, err
		}
