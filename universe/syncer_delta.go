...
// ErrJournalRewind reports that the remote's journal tail sits below
// the cursor the sync started from: the journal the cursor was
// recorded against no longer exists in that form, e.g. the remote was
// restored from a backup, or a different instance now answers at the
// same address. The caller should reset its cursor to Tail and run a
// full enumeration sync to reconcile.
type ErrJournalRewind struct {
	// Cursor is the cursor the delta sync started from.
	Cursor uint64

	// Tail is the journal tail the remote reported.
	Tail uint64
}

// Error implements the error interface.
func (e *ErrJournalRewind) Error() string {
	return fmt.Sprintf("remote journal tail %d below sync cursor %d",
		e.Tail, e.Cursor)
}

// maxDeltaPages bounds the number of pages a single delta sync run
// will consume. An honest remote needs one page per RequestPageSize
// new leaves, so the bound only trips on a multi-million-leaf backlog
// — or on a remote feeding endless artificial pages. Either way the
// run errors out and the caller falls back to enumeration sync, which
// handles both cases correctly.
const maxDeltaPages = 10_000
...
// leaf skipped under a since-changed export config) heals only when
// that universe next gains a leaf, or when a full enumeration sync
// runs. The federation envoy bounds this by forcing a periodic
// enumeration audit per server (see FederationConfig.SyncAuditInterval)
// and by resetting the cursor on ErrJournalRewind, so divergence the
// delta cannot see survives at most one audit interval.
func (s *SimpleSyncer) SyncUniverseDelta(ctx context.Context,
	...
	for pages := 0; ; pages++ {
		if pages >= maxDeltaPages {
			return nil, fmt.Errorf("delta sync exceeded %d pages "+
				"(cursor=%d)", maxDeltaPages, cursor)
		}
		...
		// round trip. A page that made progress always reports a
		// strictly greater value, so anything at or below the cursor
		// terminates the loop; refusing to walk the cursor backwards
		// also denies a misbehaving remote an endless run.
		if page.LatestSeq <= cursor {
			// A tail strictly below the cursor means the journal
			// the cursor was recorded against is gone; hand the
			// caller the rewind signal so it can reset the
			// cursor and reconcile via enumeration sync. Items
			// applied earlier in the run were individually
			// verified, so no rollback is needed.
			if page.LatestSeq < cursor {
				return nil, &ErrJournalRewind{
					Cursor: cursor,
					Tail:   page.LatestSeq,
				}
			}
