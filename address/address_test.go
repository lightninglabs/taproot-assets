...
func TestAddressUnknownOddType(t *testing.T) {
	...
	err = knownAddr.Encode(&buf)
	require.NoError(t, err)

	// With the known address now encoded, we can add an unknown even type
	// to the encoded bytes. That should provoke an error when parsed again.
	...
	err = parsedAddr.Decode(&buf)
	require.ErrorAs(t, err, &asset.ErrUnknownType{})

	// Now clear the buffer, encode the address again, but this time add an
	...
	require.Equal(t, expectedUnknownTypes, parsedAddr.unknownOddTypes)
