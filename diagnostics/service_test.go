...
func TestServiceWritesFailureArtifacts(t *testing.T) {
	...
	service, err := NewService(t.TempDir(), "test-version")
	require.NoError(t, err)
	require.NoError(t, service.Start())
	t.Cleanup(func() {
		require.NoError(t, service.Stop())
	...
	metadataBytes, err := os.ReadFile(
		filepath.Join(eventDir, "metadata.json"),
	)
	require.NoError(t, err)

	var metadata storedFailureMetadata
	require.NoError(t, json.Unmarshal(metadataBytes, &metadata))
	require.Equal(t, "test-version", metadata.TapdVersion)
