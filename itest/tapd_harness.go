...
func newTapdHarness(t *testing.T, ht *harnessTest, cfg tapdConfig,
	...
	// Sweep orphan UTXOs.
	if opts.sweepOrphanUtxos {
		args = append(args, "--wallet.sweep-orphan-utxos")
	}
...
func (hs *tapdHarness) addLogFile() error {
	...
	// Use the LND node's name to identify which tapd instance this is
	// (e.g., "Alice", "Bob").
	nodeName := hs.cfg.LndNode.Cfg.Name
	fileName := fmt.Sprintf("%s/tapd-%s.log", dir, nodeName)
