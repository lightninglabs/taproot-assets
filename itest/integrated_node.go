...
type IntegratedNode struct {
	...
	// backupDir holds the path to a database backup directory, if one has
	// been created via StopAndBackupDB.
	backupDir string
...
func (n *IntegratedNode) Restart() {
	...
	// Remove the ready file so Start() waits for the new instance to
	// create it.
	if n.readyFile != "" {
		_ = os.Remove(n.readyFile)
	}
