...
func testUniverseSync(t *harnessTest) {
	...
	// Convert asset ID bytes into a fixed size array.
	var assetIdFixedSize [32]byte
	if len(firstAssetID) <= len(assetIdFixedSize) {
		copy(assetIdFixedSize[:], firstAssetID)
	} else {
		// Only copy the first 32 bytes.
		copy(assetIdFixedSize[:], firstAssetID[:32])
	}
