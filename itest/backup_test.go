...
func testBackupSizeFootprint(t *harnessTest) {
	...
	// Track backup sizes as we add assets, comparing full vs compact.
	...
	// Mint assets in separate batches to avoid exclusion proof
	// complexity. Each batch creates one asset in its own anchor
	// transaction.
	assetNames := []string{
		"size-test-asset-1",
		"size-test-asset-2",
	...
	for i, name := range assetNames {
		...
		// Export full (v1) backup.
		fullResp, err := t.tapd.ExportAssetWalletBackup(
			ctxt, &wrpc.ExportAssetWalletBackupRequest{
				Compact: false,
			},
		)
		...
		// Export compact (v2) backup.
		compactResp, err := t.tapd.ExportAssetWalletBackup(
			ctxt, &wrpc.ExportAssetWalletBackupRequest{
				Compact: true,
			},
		)
		...
		require.Less(t.t, len(compactResp.Backup),
			len(fullResp.Backup),
			"compact backup should be smaller than full")

		metrics = append(metrics, sizeMetric{
			numAssets:    i + 1,
			fullBytes:    len(fullResp.Backup),
			compactBytes: len(compactResp.Backup),
		})
	}

	// Log the size comparison table.
	t.Logf("=== Backup Size: Full vs Compact ===")
	t.Logf("%-8s %-12s %-12s %-12s %-8s",
		"Assets", "Full", "Compact", "Savings", "%")

	for _, m := range metrics {
		savings := m.fullBytes - m.compactBytes
		pct := float64(savings) / float64(m.fullBytes) * 100
		t.Logf("%-8d %-12s %-12s %-12s %-8.1f",
			m.numAssets,
			formatBytes(m.fullBytes),
			formatBytes(m.compactBytes),
			formatBytes(savings),
			pct,
		)
	}
...
// verifying that proof chains grow with each transfer, that stale backups are
// detected, and that imported assets can be spent.
//
// Flow:
...
//  4. Import Charlie's backup on a fresh tapd (same LND), verify assets
//  5. Import Alice's and Bob's stale backups, expect 0 imported (spent)
//  6. Send from restored node to Eve to verify spendability
