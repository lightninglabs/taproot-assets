...
func runBackupFileReissuance(t *harnessTest, anchor, reissued *taprpc.Asset,
	...
	// A file whose entries describe the group correctly but whose proofs
	// are unusable, say truncated, must leave nothing behind. The group
	// is only written once a leaf of it actually imports. Creating an
	// address for the group is the probe: without a universe server the
	// node can only do that for a group its database knows.
	for _, ab := range bobBackup.Assets {
		ab.StrippedProofFileBlob = []byte("truncated")
	}
	corruptRaw, err := backup.EncodeWalletBackup(bobBackup)
	require.NoError(t.t, err)

	importResp, err := bobTapd2.ImportAssetsFromBackup(
		ctxt, &wrpc.ImportAssetsFromBackupRequest{Backup: corruptRaw},
	)
	require.NoError(t.t, err)
	require.Equal(t.t, uint32(1), importResp.NumSkipped)
	require.Equal(t.t, uint32(0), importResp.NumImported)

	_, err = bobTapd2.NewAddr(ctxt, &taprpc.NewAddrRequest{
		AddressVersion: taprpc.AddrVersion_ADDR_VERSION_V2,
		GroupKey:       groupKey,
	})
	require.ErrorContains(t.t, err, "unable to find asset or group")
	...
	// The imported leaf made the group known to the wallet.
	_, err = bobTapd2.NewAddr(ctxt, &taprpc.NewAddrRequest{
		AddressVersion: taprpc.AddrVersion_ADDR_VERSION_V2,
		GroupKey:       groupKey,
	})
