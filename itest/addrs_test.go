...
// testAddrReceivesTimestampFiltering tests the timestamp
...
func testAddrReceivesTimestampFiltering(t *harnessTest) {
	...
	// Test 2: Filter by timestamp before the send (should return events)
	resp, err = bob.AddrReceives(
		ctxt, &taprpc.AddrReceivesRequest{
			CreatedAfter: uint64(timeBeforeSend.Unix()),
		},
	)
	require.NoError(t.t, err)
	require.Len(t.t, resp.Events, 1)

	// Test 3: Filter by timestamp after the send (should return no events)
	resp, err = bob.AddrReceives(
		ctxt, &taprpc.AddrReceivesRequest{
			CreatedAfter: uint64(timeAfterSend.Unix() + 1),
		},
	)
	require.NoError(t.t, err)
	require.Len(t.t, resp.Events, 0)

	// Test 4: Filter by timestamp exactly at the send time
	...
	// Test 5: Filter by address and timestamp
	resp, err = bob.AddrReceives(
		ctxt, &taprpc.AddrReceivesRequest{
	...
	// Test 6: Filter by address and timestamp after send
