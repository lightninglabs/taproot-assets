...
func NewPreimageFromTapscriptTreeNodes(
	...
	maybeLeaves := asset.GetLeaves(tn).UnwrapToPtr()
	if maybeLeaves != nil {
		leaves := asset.ToLeaves(*maybeLeaves)
	...
	maybeBranch := asset.GetBranch(tn).UnwrapToPtr()
	if maybeBranch != nil {
		branches := asset.ToBranch(*maybeBranch)
