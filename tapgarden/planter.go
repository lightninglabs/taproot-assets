...
func (c *ChainPlanter) prepSeedlingUniCommitments(ctx context.Context,
	...
	//
	// TODO(ffranr): Ensure that this does not conflict with the
	//  values in req.Meta.Data.
	...
	if req.EnableEmission && req.GroupAnchor == nil {
		// TODO(ffranr): Handle the case where the delegation key is
		//  already specified in req.Meta.Data. Currently, we assume
		//  that it is not.
		...
		req.Meta.DelegationKey = fn.Some(*newKey.PubKey)
	}
