...
//
// Construct via NewPreCommitBindData so the required InternalKey
// invariant is enforced at the boundary. Zero-value construction
// would nil-deref downstream in the store.
...
// NewPreCommitBindData constructs a PreCommitBindData after
// validating that the InternalKey carries a non-nil PubKey. The
// store's upsert path serializes that pubkey directly; a
// zero-value key would nil-deref there.
func NewPreCommitBindData(outputIndex uint32,
	internalKey keychain.KeyDescriptor,
	groupKey fn.Option[btcec.PublicKey]) (PreCommitBindData, error) {

	if internalKey.PubKey == nil {
		return PreCommitBindData{}, fmt.Errorf(
			"pre-commit bind data: internal key pubkey is nil")
	}
...
type BatchStore interface {
	...
	CommitBatchFunding(ctx context.Context, batch *MintingBatch,
