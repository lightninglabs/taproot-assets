...
func (c *Custodian) handleMailboxMessages(msgs *mbox.ReceivedMessages) error {
	...
	for _, mboxMsg := range msgs.Messages {
		...
		if walletTx == nil {
			return fmt.Errorf("unable to find transaction %s "+
				"in block %s", fragment.OutPoint.Hash,
		...
		ctxt, cancel := c.CtxBlocking()
		event, err := c.cfg.AddrBook.GetOrCreateEvent(
			ctxt, address.StatusTransactionConfirmed, tapAddr,
			walletTx, op.Index, fragment.BlockHeight, &op.Hash,
			outputs,
		)
		cancel()
		if err != nil {
