	...
	for idx, out := range walletTx.OutputDetails {
		...
		if ok {
			...
			if event.ConfirmationHeight == 0 &&
				...
				evtSrc, err := address.NewSourceFromWalletTx(
...
func (c *Custodian) mapToTapAddr(walletTx *lndclient.Transaction,
	...
	ctxt, cancel = c.CtxBlocking()
	event, err := c.cfg.AddrBook.GetOrCreateEvent(ctxt, status, evtSrc)
	cancel()
	if err != nil {
...
func (c *Custodian) handleMailboxMessages(msgs *mbox.ReceivedMessages) error {
	...
	for _, mboxMsg := range msgs.Messages {
		...
		eventSrc, err := address.NewSourceFromFragment(
			tapAddr, block, fragment,
		)
		if err != nil {
			return fmt.Errorf("error creating event source "+
				"from fragment: %w", err)
		}
...
func AddrMatchesAsset(addr *address.AddrWithKeyInfo, a *asset.Asset) bool {
	...
	if err != nil {
		// If we can't derive the Pedersen version, we'll just use the
		// key in the address.
		pedersenScriptKey = &addr.ScriptKey
	}
