...
func testFinalizeWithTapscriptTree(t *mintingTestHarness) {
	...
	batchTapTree := input.TapscriptFullTree(
		sigLockKey, hashLockLeaf, sigLeaf,
	)
	finalizeReq = tapgarden.FinalizeParams{
		TapTree: batchTapTree,
	}
	...
	tapTreeDummyTapCommitment := input.TapscriptFullTree(
		sigLockKey, txscript.NewBaseTapLeaf(dummyTapCommitmentRootHash),
	)
	finalizeReq.TapTree = tapTreeDummyTapCommitment
	...
	seedlings = t.queueInitialBatch(numSeedlings)
	finalizeReq.TapTree = batchTapTree
	t.finalizeBatch(&wg, respChan, &finalizeReq)
	batchCount++
