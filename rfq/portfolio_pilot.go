...
func (c *InternalPortfolioPilotConfig) Validate() error {
	...
	if c.AcceptPriceDeviationPpm == 0 {
		return fmt.Errorf("AcceptPriceDeviationPpm must be > 0")
	}
...
func (p *InternalPortfolioPilot) VerifyAcceptQuote(ctx context.Context,
	...
	if resp == nil {
		return PriceOracleQueryErrQuoteRespStatus, fmt.Errorf(
			"price oracle returned nil response",
		)
	}

	// nolint: nilerr
	if resp.Err != nil {
		log.Errorf("Price oracle returned error: %v", resp.Err)
