...
func (n *Negotiator) queryBidFromPriceOracle(peer route.Vertex,
	...
	assetAmount uint64) (*rfqmsg.BigIntFixedPoint, uint64, error) {
...
func (n *Negotiator) queryAskFromPriceOracle(peer *route.Vertex,
	...
	suggestedAssetRate fn.Option[rfqmsg.BigIntFixedPoint]) (
	*rfqmsg.BigIntFixedPoint, uint64, error) {
...
func (n *Negotiator) HandleIncomingBuyRequest(
	...
	go func() {
		...
		var suggestedAssetRate fn.Option[rfqmsg.BigIntFixedPoint]
		if request.BidPrice > 0 {
			suggestedAssetRate = fn.Some(rfqmsg.NewBigIntFixedPoint(
				uint64(request.BidPrice), 0,
			))
		}
