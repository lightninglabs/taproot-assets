...
func (n *Negotiator) HandleOutgoingBuyOrder(buyOrder BuyOrder) error {
	...
	go func() {
		...
		// skip this step.
		var assetRateBid *rfqmsg.BigIntFixedPoint

		if n.cfg.PriceOracle != nil {
			// Query the price oracle for a bid price.
			var err error
			assetRateBid, _, err = n.queryBidFromPriceOracle(
...
func (n *Negotiator) queryAskFromPriceOracle(peer *route.Vertex,
	...
	suggestedAssetRate *rfqmsg.BigIntFixedPoint) (*rfqmsg.BigIntFixedPoint,
	uint64, error) {
...
func (n *Negotiator) HandleOutgoingSellOrder(order SellOrder) {
	...
	go func() {
		...
		// consideration. If a price oracle is not specified we will
		// skip this step.
		var assetRate *rfqmsg.BigIntFixedPoint
