...
// TestResolveRequestPriceOracleUnavailable ensures requests are rejected
// when no price oracle is configured, for both buy and sell paths.
func TestResolveRequestPriceOracleUnavailable(t *testing.T) {
	tests := []struct {
		// name describes the subtest case.
		name string

		// request is the RFQ request under test.
		request rfqmsg.Request
	}{
		{
			name: "buy request",
			request: func() rfqmsg.Request {
		...
		},
		{
			...
			request: func() rfqmsg.Request {
				...
				req, err := rfqmsg.NewSellRequest(
					route.Vertex{0x04, 0x05, 0x06},
					assetSpec, lnwire.MilliSatoshi(2500),
					fn.None[rfqmsg.AssetRate](),
					"order-metadata",
				)
				require.NoError(t, err)
	...
	for _, tc := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			pilot := NewInternalPortfolioPilot(
				InternalPortfolioPilotConfig{},
			)
			resp, err := pilot.ResolveRequest(ctx, tc.request)
			require.NoError(t, err)
			require.True(t, resp.IsReject())
			require.False(t, resp.IsAccept())

			var rejectErr rfqmsg.RejectErr
			resp.WhenReject(func(err rfqmsg.RejectErr) {
				rejectErr = err
			})
			require.Equal(
				t, rfqmsg.ErrPriceOracleUnavailable, rejectErr,
			)
		})
	}
}
