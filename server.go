...
func (s *Server) HtlcSigHashType(
	...
	if req.ChanID.IsSome() && s != nil && s.cfg != nil &&
		...
		srvrLog.Debugf("HtlcSigHashType called for "+
			"chan_id=%x, "+
			"deterministic_htlcs_negotiated=%v",
...
func (s *Server) htlcSigHashFromBlob(
	...
	if decErr := c.Decode(bytes.NewReader(blob)); decErr != nil {
		srvrLog.Errorf("HtlcSigHashType: unable to decode "+
			"commit blob (%d bytes): %v", len(blob), decErr)

		return lfn.None[txscript.SigHashType]()
	}

	if c.SigHashDefault.Val {
		srvrLog.Debugf("HtlcSigHashType: using cached " +
			"SigHashDefault from commit blob")
...
// NotifyBroadcast is called by lnd's sweeper to notify us of a sweep
// transaction broadcast, generated by the passed BumpRequest.
//
// NOTE: This method is part of the sweep.AuxSweeper interface.
